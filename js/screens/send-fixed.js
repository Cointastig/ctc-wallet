// js/screens/send-fixed.js - Enhanced Send Transaction Screen with Blockchain Compatibility
...
class SendScreen {
    constructor() {
        ...
        this.transactionBuilder = null; // FIX: Transaction Builder für Blockchain-Kompatibilität
        this.estimatedFee = 0;
        this.selectedPriority = 1.0; // Standard Priorität
    }
    ...
    show() {
        ...
        // FIX: Initialisiere Transaction Builder
        this.transactionBuilder = ApiUtils.createTransactionBuilder(wallet);
        
        // Lade initiale Gebührenschätzung
        ...
        const content = `
                ...
                <div class="send-form">
                    ...
                    <div class="input-group">
                        ...
                        <div class="available-balance">
                            Available: <span id="available-balance">${ApiUtils.formatCTC(this.availableBalance)}</span>
                        </div>
                    </div>
                    
                    <!-- FIX: Enhanced Transaction Fee Section -->
                    <div class="input-group">
                        <label class="input-label">Transaction Priority</label>
                        <div class="priority-selector" style="display: flex; gap: 8px; margin-bottom: 16px;">
                            <button class="priority-btn active" data-priority="0.5" onclick="sendScreen.setPriority(0.5)">
                                Economy
                            </button>
                            <button class="priority-btn" data-priority="1.0" onclick="sendScreen.setPriority(1.0)">
                                Standard
                            </button>
                            <button class="priority-btn" data-priority="2.0" onclick="sendScreen.setPriority(2.0)">
                                Fast
                            </button>
                        </div>
                        ...
                        <div style="background: rgba(255,255,255,0.05); border-radius: 12px; padding: 16px;">
                            <div style="display: flex; justify-content: space-between; align-items: center;">
                                ...
                                <span id="estimated-fee" style="color: white; font-weight: 600;">
                                    ${ApiUtils.formatCTC(this.estimatedFee)}
                                </span>
                            </div>
                            <div id="fee-usd" style="color: rgba(255,255,255,0.6); font-size: 14px; margin-top: 4px;">
                                ~$${(this.estimatedFee / 100000000 * (walletScreen.getCTCPrice() || 0.05)).toFixed(6)} USD
                            </div>
                            <div id="fee-time" style="color: rgba(255,255,255,0.6); font-size: 12px; margin-top: 4px;">
                                Expected confirmation: ${this.getConfirmationTime()}
                            </div>
                        </div>
                    </div>
                    ...
                    <!-- FIX: Enhanced Transaction Summary -->
                    <div class="card" id="transaction-summary" style="display: none;">
                        ...
                        <div style="margin-bottom: 12px;">
                            ...
                            <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                                ...
                                <span id="summary-fee" style="color: white;">${ApiUtils.formatCTC(this.estimatedFee)}</span>
                            </div>
                            <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                                <span style="color: rgba(255,255,255,0.6);">Priority:</span>
                                <span id="summary-priority" style="color: white;">Standard</span>
                            </div>
                        ...
                        <!-- FIX: UTXO Information -->
                        <div id="utxo-info" style="margin-top: 12px; padding-top: 12px; border-top: 1px solid rgba(255,255,255,0.1);">
                            <div style="display: flex; justify-content: space-between; font-size: 14px; color: rgba(255,255,255,0.6);">
                                <span>Inputs:</span>
                                <span id="input-count">-</span>
                            </div>
                            <div style="display: flex; justify-content: space-between; font-size: 14px; color: rgba(255,255,255,0.6);">
                                <span>Outputs:</span>
            ...
            <style>
                ...
                .priority-btn.active {
                    ...
                    border-color: rgba(107, 158, 255, 0.6);
                    color: #6B9EFF;
                }
            </style>
        `;
    ...
    // FIX: Lade Gebührenschätzung von der Blockchain
    async loadFeeEstimate() {
        try {
            if (this.transactionBuilder) {
                // Schätze Gebühr für Standard-Transaktion
                const dummyInputs = [{ txId: '', index: 0, address: '', amount: 100000000 }];
                const dummyOutputs = [{ address: '', amount: 50000000 }];
                
                this.estimatedFee = await this.transactionBuilder.calculateFee(
                    'transfer', 
                    dummyInputs, 
                    dummyOutputs, 
                ...
                // Update UI wenn bereits angezeigt
                const feeElement = DOM.get('estimated-fee');
                if (feeElement) {
                    feeElement.textContent = ApiUtils.formatCTC(this.estimatedFee);
                }
                
                const feeUsdElement = DOM.get('fee-usd');
                if (feeUsdElement) {
                    const usdValue = (this.estimatedFee / 100000000 * (walletScreen.getCTCPrice() || 0.05)).toFixed(6);
                    feeUsdElement.textContent = `~${usdValue} USD`;
                }
            }
        } catch (error) {
            console.warn('⚠️ Failed to load fee estimate:', error);
            this.estimatedFee = 1000; // Fallback
        }
    }

    // FIX: Setze Transaktions-Priorität
    setPriority(multiplier) {
        this.selectedPriority = multiplier;
        ...
        const activeBtn = document.querySelector(`[data-priority="${multiplier}"]`);
        if (activeBtn) {
        ...
        // Update Summary
        const priorityText = multiplier === 0.5 ? 'Economy' : 
                           multiplier === 1.0 ? 'Standard' : 'Fast';
        const summaryPriority = DOM.get('summary-priority');
        if (summaryPriority) {
            summaryPriority.textContent = priorityText;
        }
    }

    // Bestimme Bestätigungszeit basierend auf Priorität
    getConfirmationTime() {
        switch (this.selectedPriority) {
            case 0.5: return '3-5 minutes';
            case 1.0: return '1-3 minutes';
            case 2.0: return '< 1 minute';
            default: return '1-3 minutes';
        }
    }

    // Enhanced QR Scanner mit real camera access
    async showQRScanner() {
        const content = `
            <div class="wallet-container">
                <!-- Header -->
    ...
    showQRPaymentInfo(qrData) {
        ...
        let details = `<strong>Recipient:</strong> ${Utils.formatAddress(qrData.address)}<br>`;
        
        if (qrData.amount) {
            details += `<strong>Amount:</strong> ${ApiUtils.formatCTC(qrData.amount * 100000000)}<br>`;
        }
    ...
    updateAvailableBalance() {
        ...
        if (balanceElement) {
            balanceElement.textContent = ApiUtils.formatCTC(this.availableBalance);
        }
    }

    // Set maximum available amount
    setMaxAmount() {
        const maxAmount = Math.max(0, (this.availableBalance - this.estimatedFee) / 100000000);
        
        DOM.setValue('send-amount', maxAmount.toFixed(8));
        this.validateForm();
    }

    // FIX: Enhanced form validation with UTXO checking
    async validateForm() {
        ...
        // Validate address
        const addressValid = this.validateCTCAddress(address);
        
        // Validate amount
        const amountMicroCTC = Math.floor(amount * 100000000);
        const total = amountMicroCTC + this.estimatedFee;
        ...
        // FIX: Show/hide enhanced transaction summary with UTXO info
        if (addressValid && amountValid && amount > 0) {
            DOM.show(summary);
            await this.updateTransactionSummary(amountMicroCTC, total);
        } else {
            DOM.hide(summary);
        }
        ...
        if (amount > 0 && !amountValid) {
            if (total > this.availableBalance) {
                Toast.error('Insufficient balance (including network fee)');
            }
        }
    }

    // FIX: Enhanced transaction summary with UTXO information
    async updateTransactionSummary(amountMicroCTC, total) {
        try {
            // Update basic amounts
            const summaryAmount = DOM.get('summary-amount');
            const summaryFee = DOM.get('summary-fee');
            const summaryTotal = DOM.get('summary-total');
            
            if (summaryAmount) {
                summaryAmount.textContent = ApiUtils.formatCTC(amountMicroCTC);
            }
            
            if (summaryFee) {
                summaryFee.textContent = ApiUtils.formatCTC(this.estimatedFee);
            }
            
            if (summaryTotal) {
                summaryTotal.textContent = ApiUtils.formatCTC(total);
            }
            
            // FIX: Load and display UTXO information
            if (this.transactionBuilder) {
                try {
                    const wallet = walletScreen.getCurrentWallet();
                    const utxos = await ApiUtils.getInstance().getUTXOs(wallet.getAddress());
                    
                    // Simulate input selection
                    const { inputs } = this.transactionBuilder.selectInputs(utxos, amountMicroCTC);
                    
                    // Calculate outputs (recipient + change if needed)
                    const totalInput = inputs.reduce((sum, input) => sum + input.amount, 0);
                    const change = totalInput - amountMicroCTC - this.estimatedFee;
                    const outputs = change > this.transactionBuilder.DUST_LIMIT ? 2 : 1;
                    
                    // Update UTXO info
                    const inputCount = DOM.get('input-count');
                    const outputCount = DOM.get('output-count');
                    
                    if (inputCount) {
                        inputCount.textContent = `${inputs.length} UTXOs`;
                    }
                    
                    if (outputCount) {
                        outputCount.textContent = `${outputs} outputs`;
                    }
                    
                } catch (error) {
                    console.warn('⚠️ Could not load UTXO info:', error);
                    // Fallback values
                    const inputCount = DOM.get('input-count');
                    const outputCount = DOM.get('output-count');
                    
                    if (inputCount) inputCount.textContent = 'Calculating...';
                    if (outputCount) outputCount.textContent = 'Calculating...';
                }
            }
    ...
    // Enhanced CTC address validation
    validateCTCAddress(address) {
        ...
        return address.startsWith('CTC') && address.length === 24 && /^CTC[a-fA-F0-9]{21}$/.test(address);
    }

    // FIX: Enhanced transaction sending with blockchain compatibility
    async sendTransaction() {
        ...
        try {
            ...
            let confirmText = `Send ${ApiUtils.formatCTC(amountMicroCTC)} to:\n${Utils.formatAddress(address)}\n\n` +
                            `Network fee: ${ApiUtils.formatCTC(this.estimatedFee)}\n` +
                            `Priority: ${this.selectedPriority === 0.5 ? 'Economy' : this.selectedPriority === 1.0 ? 'Standard' : 'Fast'}\n` +
                            `Total: ${ApiUtils.formatCTC(total)}\n\n`;
            ...
            // FIX: Create transaction using enhanced transaction builder
            const transaction = await this.transactionBuilder.createTransfer(
                address,
                amountMicroCTC,
                this.selectedPriority
            );

            // FIX: Validate transaction before sending
            const validation = this.transactionBuilder.validateTransaction(transaction);
            ...
            const result = await ApiUtils.getInstance().sendTransaction(transaction);

            // Show success
            Toast.success(`Transaction sent successfully! TXID: ${result.txid?.substring(0, 8)}...`);
    ...
    async pasteFromClipboard() {
        try {
            ...
            if (text && this.validateCTCAddress(text.trim())) {
    ...
    getFormData() {
        return {
            ...
            qrData: this.qrData,
            isValid: this.recipientAddress && this.amount > 0 && this.validateCTCAddress(this.recipientAddress)
        };
    }
...
// FIX: Enhanced Transaction Manager Integration
class TransactionManager {
    // Send transaction using the new transaction builder
    static async sendTransaction(fromAddress, toAddress, amount, wallet) {
        try {
            const transactionBuilder = ApiUtils.createTransactionBuilder(wallet);
            
            // Convert amount to micro-CTC if it's a decimal
            const amountMicroCTC = typeof amount === 'number' && amount < 1000000 
                ? Math.floor(amount * 100000000) 
                : amount;
            
            // Create and send transaction
            const transaction = await transactionBuilder.createTransfer(toAddress, amountMicroCTC);
            const result = await ApiUtils.getInstance().sendTransaction(transaction);
            
            return result;
            
        } catch (error) {
            console.error('❌ TransactionManager error:', error);
            throw error;
        }
    }
}
...
console.log('📤 Enhanced Send screen with Blockchain Compatibility loaded');
